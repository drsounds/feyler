...
void Win32GraphicsContext::invalidateRegion(rectangle rect) {
...
bounds Win32GraphicsContext::getBounds() {
	RECT clientRect;
...
Image *Win32GraphicsContext::loadImage(const string& _bitmap) {
    ...
    MessageBox(0, a.c_str(), L"Path", MB_ICONSTOP | MB_OK);
    cout << "Bitmap path: " << a << "\r\n";
...
#define ANTIALIASED_QUALITY 0x04
    ...
    GetTextExtentPoint32(this->hDC, (LPCWSTR)text, len, &sizeText);
    DeleteObject(font);
...
#define ANTIALIASED_QUALITY 0x04
    ...
    DrawText(this->hDC, (LPCSTR)text, lstrlen((LPCWSTR)text), &rect, 0);
    DeleteObject(font);
}
...
void Win32GraphicsContext::drawControl(char *control, Element *elm, int x, int y, int w, int h) {
    ...
    if (count_controls > 0) {
        ...
        hwnd = CreateWindow((LPCWSTR)control, 0, WS_CHILD|WS_VISIBLE, x, y / 4, w, h / 4, this->hWnd, 0, NULL, 0);
        controls->insert(std::pair<Element *, HWND>(elm, hwnd));
        HFONT font = CreateFont(8, 4, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_CHARACTER_PRECIS,  CLIP_MASK, CLEARTYPE_QUALITY , FF_MODERN | VARIABLE_PITCH, (LPCWSTR)"MS Sans Serif");
...
void Win32GraphicsContext::drawInputElement(Element *elm, int x, int y, int w, int h) {
    ...
    this->drawControl((char *)"EDIT", elm, x, y, w, h);
}
